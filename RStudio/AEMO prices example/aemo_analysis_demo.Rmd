...
Figure \ref{fig:fig_prices_max_temp}. 
...
plot(df$temp_min, df$RRP, xlab = "Max Temp (C)", ylab = "RRP (AUD $)", title("Electricity Prices"))
```
...
linear model \eqref{eqt:linear_model} has the $p$-value $`r linear_model_p_value`$, 
for which is significant. Furthermore, trying a higher order non-linear model
with quadratic terms achieves $p$-values of $`r non_linear_model_p_value`$, for the
...
Which when given $x = (1,2,\ldots,10)$ and knot points $(2,6)$ will output the
model matrix:
